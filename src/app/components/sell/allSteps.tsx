...
const getVariantsForModel = (brand: string, model: string, fuel: string): VariantType[] => {
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 1:
  return (
    <div className="w-full max-w-6xl mx-auto mt-[70px] px-4">
      {/* Heading */}
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-white font-heading tracking-tight mb-2">
          Choose Your Car Brand
        </h2>
        <p className="text-sm text-gray-400 font-clean">
          Select the manufacturer of your vehicle to begin
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {brands.map((brand) => (
          <button
            ...
            className={`
              relative flex flex-col items-center justify-center p-5 rounded-2xl transition-transform duration-300 ease-in-out
              border backdrop-blur-sm
              ${
                selectedBrand === brand.name
                  ? 'border-[#D4AF37] bg-[#D4AF37]/10 shadow-xl shadow-[#D4AF37]/30 scale-105'
                  : 'border-gray-700 hover:border-[#D4AF37]/40 hover:scale-[1.03] bg-black/30'
              }
            `}
            style={{
              transformStyle: 'preserve-3d',
              boxShadow: selectedBrand === brand.name ? '0 0 20px rgba(212, 175, 55, 0.4)' : undefined,
            }}
          >
            {/* Logo */}
            ...
            {/* Brand Name */}
            <span className="text-sm text-white font-semibold font-secondary tracking-wide">
              {brand.name}
            </span>

            {/* Selected Pulse */}
            {selectedBrand === brand.name && (
              <span className="absolute top-2 right-2 w-3 h-3 rounded-full bg-[#D4AF37] animate-ping"></span>
            )}
          </button>
        ))}
      </div>
