...
interface Value { icon: React.ComponentType<{ className?: string }>; title: string; desc: string; }
interface TeamMember { name: string; role: string; motto: string; }
...
const AboutUs: React.FC = () => {
  ...
  const text1 = "Reimagining Pre-Owned Luxury with Purpose, Passion, and Precision.";
  const text2 = "Powered by Raam Group, Epic Luxe curates premium pre-owned luxury vehicles with unmatched quality, trust, and elegance.";
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const lineAnimation = setInterval(() => {
              ...
              if (progress >= 100) clearInterval(lineAnimation);
            }, 30);
  ...
  const team: TeamMember[] = [
    { name: 'Ravi Kumar', role: 'Founder & CEO', motto: "Luxury isn't a product—it's an emotion" },
    { name: 'Priya Sharma', role: 'Head of Operations', motto: 'Excellence in every detail' },
    { name: 'Rahul Mehta', role: 'Customer Experience Director', motto: 'Creating unforgettable journeys' },
    { name: 'Sonal Patel', role: 'Quality Assurance Head', motto: 'Perfection is our standard' }
  ];
  ...
  return (
    <div className="bg-white text-black overflow-hidden font-primary">
      <Header />
      {/* Cursor Tracking Gold Radials */}
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-48 h-48 bg-gradient-radial from-[#D4AF37]/12 to-transparent rounded-full blur-2xl animate-pulse delay-1000" />
      </div>
      ...
      <section className="relative min-h-screen flex flex-col justify-center overflow-hidden pt-[100px] sm:pt-[120px] md:pt-[140px]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-br from-[#f7f7f7]/90 via-[#ffffff]/80 to-[#f7f7f7]/90 z-10" />
        ...
        <div className="relative z-30 text-center max-w-6xl mx-auto w-full px-2 sm:px-4">
          ...
          >
            <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-[#D4AF37] via-black to-[#BFA980] bg-clip-text text-transparent tracking-wide font-accent">
              Where Luxury Meets Legacy
            </h1>
            <div className="relative mb-6">
              <p className="text-sm sm:text-base md:text-lg text-black/70 max-w-full sm:max-w-2xl mx-auto leading-relaxed font-medium font-primary min-h-[60px] flex items-center justify-center break-words">
          ...
          >
            ...
            <div className="relative mb-8">
              <p className="text-black/80 text-xs sm:text-sm md:text-base leading-relaxed max-w-full sm:max-w-3xl mx-auto px-1 sm:px-2 font-primary min-h-[60px] sm:min-h-[80px] flex items-center justify-center break-words">
                <span className="relative">
                  {typewriterText2}
                  {showCursor2 && (
          ...
          <div className="w-full mt-2 sm:mt-4">
            <div className="hidden md:grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 px-2">
              {features.map((item, i) => (
                <div
                  key={i}
                  className="bg-white border border-[#D4AF37]/30 rounded-xl p-4 flex flex-col items-center text-center hover:shadow-[0_0_12px_#D4AF37] transition-all duration-300 backdrop-blur-md transform hover:scale-105"
                >
                  <div className="text-xl lg:text-2xl mb-2">{item.icon}</div>
                  <h3 className="text-black/90 text-xs md:text-sm font-semibold mb-1 font-secondary">{item.title}</h3>
                  <p className="text-black/60 text-[11px] md:text-xs font-primary">{item.desc}</p>
                </div>
              ))}
            </div>
            {/* Features Mobile */}
                ...
                >
                  {features.map((item, i) => (
                    <div key={i} className="w-full flex-shrink-0 px-2">
                      <div className="bg-white border border-[#D4AF37]/30 rounded-xl p-5 flex flex-col items-center text-center backdrop-blur-md mx-auto max-w-xs">
                        <div className="text-2xl sm:text-3xl mb-3">{item.icon}</div>
                        <h3 className="text-black/90 text-base font-semibold mb-1 font-secondary">{item.title}</h3>
                        <p className="text-black/60 text-xs font-primary">{item.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {features.map((_, i) => (
                  <div
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      i === currentFeature ? 'bg-[#D4AF37] scale-125' : 'bg-black/20'
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="relative py-12 sm:py-16 lg:py-24 px-4 -mt-6 sm:-mt-0">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div
              data-animate
              className={`transition-all duration-1000 delay-200 ${isVisible[2] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}
            >
              <h2 className="text-5xl font-bold mb-8 text-black font-accent">Who We Are</h2>
            ...
            <div
              data-animate
              className={`transition-all duration-1000 delay-400 ${isVisible[2] ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}
            >
      ...
      {/* TIMELINE */}
      <section ref={timelineRef} className="relative py-16 px-4 bg-gradient-to-b from-[#f7f7f7] to-white">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-12 transition-all duration-1000 ${
            isLineVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
          }`}>
            <h2 className="text-4xl md:text-5xl font-light mb-4 text-black font-accent">
              Our Group Journey
            </h2>
            <p className="text-lg text-black/70 max-w-2xl mx-auto font-light font-primary">
                ...
                {milestones.map((milestone, index) => {
                  ...
                  return (
                    <div key={index} className="relative">
                      <div
                        className={`milestone-card ${isMilestoneVisible ? 'visible' : ''} bg-white shadow-md p-4 rounded-xl border border-[#D4AF37]/20 hover:border-[#D4AF37]/50 transition-all duration-500 text-center`}
                        style={{
                      ...
                      >
                        ...
                        <h3 className="text-sm font-medium text-black mb-2 font-secondary">{milestone.title}</h3>
                        <p className="text-black/60 text-xs leading-relaxed font-light font-primary">
                          {milestone.desc}
                        </p>
                      </div>
                      <div
                        className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-3 h-3 bg-gradient-to-br from-[#D4AF37] to-[#BFA980] rounded-full border-2 border-white z-10"
                        style={{
              ...
              >
                {milestones.map((milestone, index) => (
                  <div key={index} className="w-full flex-shrink-0 px-4">
                    <div className="bg-white shadow-md p-6 rounded-xl border border-[#D4AF37]/20 text-center mx-auto max-w-sm">
                      <div className="text-2xl font-semibold text-[#D4AF37] mb-3 font-secondary">{milestone.year}</div>
                      <h3 className="text-lg font-medium text-black mb-3 font-secondary">{milestone.title}</h3>
                      <p className="text-black/60 text-sm leading-relaxed font-light font-primary">{milestone.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {milestones.map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${
                    i === currentMilestone ? 'bg-[#D4AF37] scale-125' : 'bg-black/10'
                  }`}
                />
              ))}
            </div>
          ...
          <div className={`text-center mt-8 transition-all duration-1000 ${
            ...
            <p className="text-black/40 text-sm font-light font-primary">Journey continues...</p>
          </div>
        </div>
      </section>

      {/* LEADERSHIP TEAM */}
      <section className="relative py-24 px-4 bg-gradient-to-b from-[#f7f7f7] to-white">
        <div className="max-w-7xl mx-auto">
          <div data-animate className={`text-center mb-16 transition-all duration-1000 ${isVisible[4] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <h2 className="text-5xl font-bold mb-6 text-black font-accent">Leadership Vision</h2>
          </div>
          <div>
            <div className="hidden md:grid grid-cols-4 gap-6">
              {team.map((member, index) => (
                <div
                  key={index}
                  data-animate
                  className={`group relative bg-white p-4 rounded-xl border border-[#D4AF37]/20 hover:border-[#D4AF37]/50 transition-all duration-500 hover:transform hover:scale-105 ${isVisible[4] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                  style={{ transitionDelay: `${index * 100}ms` }}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-[#D4AF37]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" />
            ...
            <div className="md:hidden">
              <div className="overflow-hidden">
                ...
                >
                  {team.map((member, index) => (
                    <div key={index} className="w-full flex-shrink-0 px-4">
                      <div className="bg-white p-6 rounded-xl border border-[#D4AF37]/20 text-center mx-auto max-w-xs">
              ...
              <div className="flex justify-center mt-6 space-x-2">
                {team.map((_, i) => (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      i === currentTeamMember ? 'bg-[#D4AF37] scale-125' : 'bg-black/10'
                    }`}
                  />
                ))}
              </div>
      ...
      <section className="relative py-24 px-4">
        <div className="max-w-7xl mx-auto">
          <div data-animate className={`text-center mb-16 transition-all duration-1000 ${isVisible[5] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <p className="text-xl text-black/70 font-primary">
          ...
          <div className="hidden md:grid grid-cols-5 gap-6">
            {values.map((value, index) => (
              <div
                key={index}
                data-animate
                className={`group relative bg-white p-6 rounded-xl border border-[#D4AF37]/20 hover:border-[#D4AF37]/50 transition-all duration-500 hover:transform hover:scale-105 ${isVisible[5] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${index * 150}ms` }}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-[#D4AF37]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" />
                <div className="relative text-center">
                  <value.icon className="w-10 h-10 text-[#D4AF37] mb-4 mx-auto group-hover:scale-110 transition-transform duration-300" />
                  <h3 className="text-lg font-bold mb-3 text-black group-hover:text-[#D4AF37] transition-colors font-secondary">
                    {value.title}
                  </h3>
                  <p className="text-black/60 group-hover:text-black transition-colors font-primary text-sm">
                    {value.desc}
                  </p>
                </div>
              </div>
          ...
          <div className="md:hidden">
            <div className="overflow-hidden">
              ...
              >
                {values.map((value, index) => (
                  <div key={index} className="w-full flex-shrink-0 px-4">
                    <div className="bg-white p-8 rounded-xl border border-[#D4AF37]/20 text-center mx-auto max-w-xs">
                      <value.icon className="w-14 h-14 text-[#D4AF37] mb-6 mx-auto" />
                      <h3 className="text-xl font-bold mb-4 text-black font-secondary">{value.title}</h3>
                      <p className="text-black/60 font-primary leading-relaxed">{value.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-center mt-4">
              <div className="w-32 h-1 bg-black/10 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${((currentValue + 1) / values.length) * 100}%` }}
                />
              </div>
            </div>
          </div>
      ...
      {/* TESTIMONIALS */}
      <section className="relative py-24 px-4 bg-gradient-to-b from-white to-[#f7f7f7]">
        <div className="max-w-7xl mx-auto">
          <div data-animate className={`text-center mb-12 transition-all duration-1000 ${isVisible[6] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <h2 className="text-5xl font-bold mb-6 text-black font-accent">
            ...
            <div className="w-full md:w-1/2 mx-auto bg-white shadow-lg rounded-xl p-10 border border-[#D4AF37]/20 text-center">
              ...
              <div className="flex justify-center my-2">
                {[...Array(testimonials[currentTestimonial].rating)].map((_, i) => (
                  <Star key={i} className="w-5 h-5 text-[#D4AF37] inline" />
                ))}
              </div>
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {testimonials.map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${
                    i === currentTestimonial ? 'bg-[#D4AF37] scale-125' : 'bg-black/10'
                  }`}
                />
              ))}
            </div>
      ...
      <section className="relative py-16 px-4 bg-gradient-to-b from-[#f7f7f7] to-white">
        <div className="max-w-7xl mx-auto">
          <div 
            data-animate 
            className={`text-center mb-12 transition-all duration-1000 ${isVisible[7] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-4 text-black font-accent">
              Get Directions to Our Showroom
            </h2>
            <p className="text-xl text-black/70 font-primary">Find us easily with our interactive map and navigation</p>
          </div>
          <div 
            data-animate 
          ...
          >
            <div className="bg-gradient-to-r from-[#D4AF37]/10 via-[#D4AF37]/5 to-[#D4AF37]/10 border-2 border-[#D4AF37]/30 rounded-2xl p-6 text-center backdrop-blur-sm">
              <div className="flex items-center justify-center gap-3 mb-3">
                ...
                <h3 className="text-2xl md:text-3xl font-bold text-[#D4AF37] font-accent">Visit Our Premium Flagship Showroom</h3>
                <div className="w-2 h-2 bg-[#D4AF37] rounded-full animate-pulse"></div>
              </div>
              <div className="flex items-center justify-center gap-6 mt-4 text-sm text-black/70">
          ...
          <div className="relative">
            ...
            >
              <div className="bg-white rounded-2xl p-4 border border-[#D4AF37]/20 overflow-hidden">
                <div className="h-80 rounded-xl overflow-hidden border border-[#D4AF37]/10">
                  <iframe
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  <button
                    onClick={() =>
                      window.open(
                        'https://maps.google.com/maps?saddr=My+Location&daddr=Silver+Star+Mercedes-Benz+Hyderabad',
                        '_blank'
                      )
                    }
                    className="bg-black/5 backdrop-blur-sm border border-[#D4AF37]/30 text-black px-4 py-2 rounded-lg font-semibold hover:bg-[#D4AF37]/20 hover:border-[#D4AF37]/50 transition-all duration-300 flex items-center gap-2 font-primary text-sm"
                  >
            ...
            {/* Contact Card – Mobile (shown below map) */}
            <div
              data-animate
            ...
            >
              <div className="bg-black/5 backdrop-blur-sm rounded-xl p-6 border border-[#D4AF37]/20">
                <h3 className="text-xl font-semibold text-[#D4AF37] mb-4 font-secondary">
                  Visit Our Flagship Showroom
                </h3>
                <div className="space-y-3 text-black/70 font-primary">
                  <div className="flex items-start gap-3">
                    <MapPin className="w-5 h-5 text-[#D4AF37] mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-black font-medium">Hyderabad Flagship</p>
      ...
      {/* STICKY FOOTER CTA */}
      <section className="relative py-20 px-4 bg-gradient-to-r from-[#f7f7f7] via-[#fff] to-[#f7f7f7]">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-4xl font-bold mb-6 text-black font-accent">Own a Piece of Luxury</h2>
          <p className="text-xl text-black/70 mb-8 font-primary">Experience Raam</p>
          <div className="relative">
            ...
            <button className="relative bg-gradient-to-r from-[#D4AF37] to-[#BFA980] text-black px-12 py-6 rounded-2xl text-2xl font-bold hover:shadow-2xl hover:shadow-[#D4AF37]/50 transition-all duration-300 transform hover:scale-105 group font-primary">
              ...
              <div className="absolute inset-0 bg-black/10 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-2xl" />
            </button>
          </div>
        </div>
      </section>
      ...
      <style jsx>{`
        ...
        @keyframes fadeZoomLoop { 0% { opacity: 0; transform: scale(1.1); } 8% { opacity: 1; transform: scale(1.05); } 25% { opacity: 1; transform: scale(1); } 35% { opacity: 0; transform: scale(1); } 100% { opacity: 0; transform: scale(1); } }
        .animate-fade-zoom-loop { animation: fadeZoomLoop 12s infinite ease-in-out; transition: opacity 1s ease-in-out; }
        @keyframes kenBurns { 0% { transform: scale(1) translateX(0); } 25% { transform: scale(1.05) translateX(-2%); } 50% { transform: scale(1.08) translateY(-1%); } 75% { transform: scale(1.03) translateX(1%); } 100% { transform: scale(1) translateX(0); } }
        .animate-ken-burns { animation: kenBurns 20s ease-in-out infinite; }
        .milestone-card { transform: translateY(30px); opacity: 0; transition: all 0.8s cubic-bezier(0.4, 0, 0.2, 1); }
        .milestone-card.visible { transform: translateY(0); opacity: 1; }
