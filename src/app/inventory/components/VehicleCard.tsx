...
export default function VehicleGrid({ vehicles }: VehicleGridProps) {
      ...
      rowVehicles.forEach((car) => {
        elements.push(
          <VehicleCard
            ...
            openDetails={() => router.push(`/vehicle/${car.id}`)}
            openSchedule={() => {
            ...
            openShare={() => {
              if (navigator.share) {
                navigator.share({
                  ...
                  url: window.location.origin + `/vehicle/${car.id}`,
                }).catch(() => { });
              } else {
                window.open(window.location.origin + `/vehicle/${car.id}`, "_blank");
              }
            }}
          />
        );
  ...
  return (
    <div className="w-full bg-black pt-8 pb-0 min-h-screen">
      ...
      {compareList.length > 0 && !compareOpen && (      // ← add !compareOpen
  <button
...
function VehicleCard({
  ...
  const priceVal = Number(car.price.replace(/[^0-9]/g, ''));
    ...
    <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-gray-900 to-black border border-gray-700 shadow-2xl flex flex-col group hover:shadow-gold/20 hover:border-gold/30 transition-all duration-500 h-full">
      ...
      <div className="flex flex-col flex-grow p-5 text-white">
        ...
        <div className="space-y-3 mt-auto">
          ...
          {/* Bottom Row: Premium Contact Methods */}
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => window.open(`tel:+919900123400`, '_self')}
              className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 border border-blue-500 rounded-xl px-4 py-3 text-sm font-bold text-white transition-all duration-300 hover:from-blue-500 hover:to-blue-600 hover:shadow-lg hover:shadow-blue-500/20"
            ...
            <button
              ...
              aria-label="Schedule premium viewing"
            >
              <Calendar size={16} />
              <span>Schedule</span>
            </button>
