...
const AnimatedStep = ({ children, direction, step }: { children: React.ReactNode; direction: 'next' | 'back'; step: number }) => (
...
// Trustworthy Icons Component
const TrustworthyIcons = () => {
  const trustFeatures = [
    {
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {trustFeatures.map((feature, index) => (
          ...
          >
            ...
            <p className="text-xs md:text-sm text-gray-200/70">
              {feature.subtitle}
            </p>
          </motion.div>
        ))}
...
// Sticky Car Details Component
const CarDetailsBar = ({ formData }: { formData: any }) => {
  const details = [
    formData.brand,
    formData.model,
    formData.fuelType && formData.transmission ? `${formData.fuelType} ${formData.transmission}` : formData.fuelType,
    formData.variant,
    formData.rto,
    formData.year,
    formData.ownership,
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between">
          <span className="text-black font-semibold text-sm whitespace-nowrap">Your Car Details</span>
    ...
    <div className="min-h-screen w-full relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="w-full">
          ...
          {/* AllSteps Component - NO CONTAINERS AT ALL */}
          ...
          {/* Floating Navigation Helper (only show back button if needed) */}
          {step > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* Floating Call Button (appears from step 3) */}
          {step >= 3 && (
            <motion.div
